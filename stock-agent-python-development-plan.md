...
- 当前进度：D11 本地 RAG 基础链路及 D12 通用 Knowledge Tool 接入已完成，见 docs/day12.md；中文检索仍有失败案例。D13 已实现 SEC ticker / CIK、recent filing 发现、外国发行人表单及 as_of 筛选，历史清单与无匹配资料的具体原因遗留，见 docs/day13.md。D14 已在限定附件范围内完成 SEC HTML 主文档、6-K HTML EX-99 附件选择、正文和表格清洗、来源元数据、内容哈希、block 字符范围与 XPath，见 docs/day14.md。D15 已完成任意 ticker 的首次按需索引、进程内复用、索引配置身份、来源 URL 和按引用证据校验 data mode，见 docs/day15.md；精确 block/XPath 对外回查遗留，持久化与 freshness 分别留在 D16、D17。D08/D09 的完成与遗留项见对应文档，早期未验收项继续保留。
...
从 D16 的跨进程持久化继续推进；D13 的历史清单和无匹配资料原因仍保留为遗留项。D14 已完成原文 block、字符范围和 XPath，附件范围限制为 6-K / 6-K/A 的 HTML EX-99，其他附件不声称覆盖。D15 已完成最小按需索引和进程内复用，freshness 与增量更新仍属于 D17。D01–D15 已有代码与记录保留，未验收项仍需追踪。真实数据工程按 D13–D17 逐步落地。50 个开发日的主框架保留，D23 / D32 明确技术分析与因子衔接，不新增开发周；数据覆盖、解析或技术计算工作量超出每日时限时使用原有缓冲，不以减少支持的公司数量代替产品目标。
