...
| D08 | 在相同固定案例上比较 Manual Agent 和 LangChain Agent | 记录调用次数、错误、工具顺序差异；能解释框架接管了什么 |
| D09 | 整合结构化输出、预算、超时和事件记录 | 无 choices、截断、非法 JSON、虚构引用和预算耗尽均有明确终态 |
| D10 | 跑通 CLI / API 的最小研究路径，建立 React 结果页骨架 | 一个请求有结果、来源和运行身份；失败不会泄露原始异常 |
...
| D43 | 完善 React 问题输入和结果页面 | 显示行情时间、宏观统计期、引用和不确定性 |
...
| 方法与路径 | 用途 | 关键检查 |
| --- | --- | --- |
| POST /api/runs | 创建研究与风险分析 | 问题、标的、资料截止时间、身份和预算 |
...
- [ ] Manual Agent 与 LangChain Agent 共用只读工具契约，并有固定案例对照。
